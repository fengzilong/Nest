...
/***/ function(module, exports, __webpack_require__) {
	...
	riot.tag2('ui-pagination', '<ui-button __disabled="{current === min}" sm on-click="{onChangeFactory( current - 1 )}"> {parent.opts.prevText || \'prev\'} </ui-button> <ui-button if="{isBeforeHidden}" on-click="{onChangeFactory( min )}" sm> {parent.min} </ui-button> <span if="{isBeforeHidden}" class="{styles.more}">...</span> <ui-button if="{isBeforeHidden}" on-click="{onChangeFactory( current - 2 )}" sm> {parent.current - 2} </ui-button> <ui-button if="{isBeforeHidden}" on-click="{onChangeFactory( current - 1 )}" sm> {parent.current - 1} </ui-button> <virtual each="{v in min2current}" no-reorder> <ui-button if="{!isBeforeHidden}" on-click="{onChangeFactory( v )}" sm> {v} </ui-button> </virtual> <ui-button primary sm>{parent.current || parent.min}</ui-button> <virtual each="{v in current2max}" no-reorder> <ui-button if="{!isAfterHidden}" on-click="{onChangeFactory( v )}" sm> {v} </ui-button> </virtual> <ui-button if="{isAfterHidden}" sm on-click="{onChangeFactory( current + 1 )}"> {parent.current + 1} </ui-button> <ui-button if="{isAfterHidden}" on-click="{onChangeFactory( current + 2 )}" sm> {parent.current + 2} </ui-button> <span if="{isAfterHidden}" class="{styles.more}">...</span> <ui-button if="{isAfterHidden}" on-click="{onChangeFactory( max )}" sm> {parent.max} </ui-button> <ui-button __disabled="{current === max}" on-click="{onChangeFactory( current + 1 )}" sm> {parent.opts.nextText || \'next\'} </ui-button>', '', '', function (opts) {
		var _this = this;
	
		this.styles = _pagination2.default;
		this.min2current = [];
		this.current2max = [];
	
		this.onChangeFactory = function (v) {
			return function () {
				_this.opts.onChange && _this.opts.onChange(v);
				_this.update();
			};
		};
	
		this.on('update', function () {
			var min = parseInt(_this.opts.min) || 1;
			var max = parseInt(_this.opts.max) || 1;
			var current = parseInt(_this.opts.current) || min;
	
			_this.min = min;
			_this.max = max;
			_this.current = current;
	
			var isBeforeHidden = false;
			if (current - min > 3) {
				isBeforeHidden = true;
			}
	
			var isAfterHidden = false;
			if (max - current > 3) {
				isAfterHidden = true;
			}
	
			_this.isBeforeHidden = isBeforeHidden;
			_this.isAfterHidden = isAfterHidden;
...
/***/ function(module, exports) {
	...
	// removed by extract-text-webpack-plugin
	module.exports = {"more":"pagination__more--07727"};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _submenu = __webpack_require__(66);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _collapsePanel = __webpack_require__(87);
...
/***/ function(module, exports, __webpack_require__) {
	...
	riot.tag2('ui-tooltip', '<div name="t" class="{styles.target}"> <yield></yield> </div> <div show="{( !manual && triggered ) || ( manual && opts.show )}" name="s" class="{styles.base} {styles[ opts.placement || defaultPlaceholder ]}"> <div class="{styles.arrow}"></div> <div class="{styles.content}"> {opts.title} </div> </div>', '', '', function (opts) {
		var _this = this;
		...
		this.on('updated', function () {
			...
			if (!shallShowTooltip || !isTrackValid || typeof _this.opts.trackby === 'undefined') {
				return;
			}
	
			// TODO: trackby support array
			// if trackby changed, trigger align
			if (_this.opts.trackby !== lastTrackBy) {
				lastTrackBy = _this.opts.trackby;
...
/***/ function(module, exports, __webpack_require__) {
	...
	riot.tag2('ui-slider', '<div class="{styles.base}"> <div name="handle" class="{styles.handle}" riot-style="left: {offsetRate + \'%\'};" onmousedown="{onMouseDown}" onmouseenter="{onMouseEnter}" onmouseleave="{onMouseLeave}"></div> <div name="tracker" class="{styles.tracker}"> <div class="{styles[ \'tracker-inner\' ]}" riot-style="width: {offsetRate + \'%\'};"></div> </div> <ui-tooltip title="{tipContent}" trigger="manual" show="{showTooltip}" placement="{opts.tipPlacement || \'top\'}" track="{handle}" trackby="{offsetRate}"></ui-tooltip> </div>', '', '', function (opts) {
		var _this = this;
	
		var min = this.opts.min || 0;
		var max = this.opts.max || 100;
		var step = this.opts.step || 1;
		var stepPercent = step / (max - min) * 100;
		var value = this.opts.value || min;
		...
		this.onMouseDown = function (e) {
			...
			var onMouseMove = function onMouseMove(e) {
				...
				var moved = offsetX / trackerWidth * 100;
				var inferOffset = Math.round((initOffsetRate + moved) / stepPercent) * stepPercent;
	
				// 计算出的下一个step点需要与移动方向一致
				if (moveDirection < 0 && inferOffset - _this.offsetRate < 0 || moveDirection > 0 && inferOffset - _this.offsetRate > 0) {
					_this.offsetRate = inferOffset;
				}
...
/***/ function(module, exports) {
	...
	// removed by extract-text-webpack-plugin
	module.exports = {"base":"slider__base--93502","handle":"slider__handle--fd370","tracker":"slider__tracker--26f65","tracker-inner":"slider__tracker-inner--4dcf9"};
